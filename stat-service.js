    ...
    for (let gameId of gameIds) {
        try {
            const response = await axios.get(
                nbaDataUrlPrefix + date + '/' + gameId + boxscoreSuffix
            )
...
const getCurrentStandingsByDate = async (date, rosterFile, standingFile) => {
    ...
    if (searchDate.isAfter(lastUpdate)) {
        ...
        if (playerStats.size > 0) {
            ...
            teams.forEach((team) => {
                let teamStanding = currentStandings.standings.find(
                    (o) => o.name === team.name
                )
            ...
            let sortedStandings = currentStandings.standings.sort(function (
                a,
                b
            ) {
            ...
            try {
                fs.writeFileSync(
                    rosterFile,
                    JSON.stringify(jamesLeague, null, 4)
                )
                fs.writeFileSync(
                    standingFile,
                    JSON.stringify(currentStandings, null, 4)
                )
            } catch (err) {
                console.error(err)
            }
...
const updateRosterInfo = async () => {
    let jamesLeague = JSON.parse(
        fs.readFileSync('store/playoff-roster.json', 'utf8')
    )
    let players = JSON.parse(
        fs.readFileSync('sample-data/players.json', 'utf8')
    )
    let nbaTeams = JSON.parse(fs.readFileSync('sample-data/teams.json', 'utf8'))
    jamesLeague.teams.forEach((team) => {
        team.roster.forEach(async (player) => {
            let pData = players.league.standard.find(
                (o) => o.personId === player.personId
            )
            if (pData) {
                ...
                let teamData = nbaTeams.league.standard.find(
                    (o) => o.teamId === pData.teamId
                )
